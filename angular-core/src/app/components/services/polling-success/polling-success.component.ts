...
@Component({
  ...
  templateUrl: './polling-success.component.html',
  styleUrl: './polling-success.component.scss'
})
export class PollingSuccessComponent {
  ...
  private async _startPolling(): Promise<IModel | null> {
    return await lastValueFrom(
      interval(this.pollDelay).pipe(
        ...
        filter((model: IModel | null) => model !== null && model.attributes.resultCode === ResultCode.SUCCESS),
        take(1),
        defaultIfEmpty(null),
        catchError(() => of(null)),
