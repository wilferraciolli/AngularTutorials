...
Type in (this will create the tsconfig file)
   ```tsc --init```
...
transpile from typescript to javascript with 
    ```tsc --outDir dist```  --To compile
    ```tsc -watch```  -- to allow live reload


# Code explanation
This program uses
    package.json - to configure the project dependencies
    tsconfig - to configure the typescript
    webpack.config - this is to configure the web server to run the app and hot reload
